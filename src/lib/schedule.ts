...
export function mapScheduleActivityToEvent(activity: ScheduleActivityApiItem, locale: string): ScheduleEvent | null {
  ...
  const end = activity.end ? new Date(activity.end) : calcEndFromDuration(start, activity.durata);
...
export function mapScheduleActivitiesToEvents(activities: ScheduleActivityApiItem[], locale: string): ScheduleEvent[] {
