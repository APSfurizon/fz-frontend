...
export class AutoInputUsersManager implements AutoInputManager {
  ...
  constructor(
    ...
    if (!getParams) {
      this.getParams = (value, additionalValues) => {
        ...
        const params: any[] = [...(additionalValues || [])];
        return buildSearchParams({ name: value, "is-admin-search": "" + ((params[0] as boolean) ?? false) });
      };
    } else {
  ...
  loadByIds(filter: AutoInputFilter): Promise<AutoInputSearchResult[]> {
    return new Promise((resolve) => {
      ...
      runRequest({ action: new GetUserByIdAction(), searchParams: params })
        .then((results) => {
...
export class UpdatePersonalInfoAdminFormAction extends FormApiAction<
  UserPersonalInfo,
  SimpleApiResponse,
  ApiErrorResponse
> {
...
export const idTypeAnswers = [
  new SelectItem(undefined, "id_card", "Id card", "ID_CARD", undefined, undefined, {
    "it-it": "Carta d'identità",
    "en-gb": "Identity Card",
  }),
  new SelectItem(undefined, "driver_license", "Driver license", "DIRECTIONS_CAR", undefined, undefined, {
    "it-it": "Patente",
    "en-gb": "Driver license",
  }),
  new SelectItem(undefined, "passport", "Passport", "PERSON_BOOK", undefined, undefined, {
    "it-it": "Passaporto",
    "en-gb": "Passport",
  }),
];

export const shirtSizeAnswers = ["xs", "s", "m", "l", "xl", "xxl", "3xl"].map(
  (sz) => new SelectItem(undefined, sz, sz.toUpperCase(), undefined, undefined, undefined, undefined)
);
