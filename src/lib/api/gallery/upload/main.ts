...
export class GalleryUpload {
  ...
  private dispatchEvent(eventType: GalleryUploadEvent, params?: Omit<GalleryUploadEventParams, "data">) {
    ...
    if (this.eventHandlers[eventType]) {
      Promise.all(
        this.eventHandlers[eventType].map(
          (callback) =>
            new Promise((resolve) => {
              try {
  ...
  private uploadAllChunks(chunks: ChunkUpload[]): Promise<ChunkUpload[]> {
    ...
    // Progress handling
    // map chunks
    this.chunkStatusMap = chunks.reduce(
      (record, chunk) => {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        for (const toUpload of batchedChunks) {
          const executed = await Promise.all(toUpload.map((chunk) => this.uploadChunk(chunk)));
...
export const copyrightValues: SelectItem[] = [
  new SelectItem(undefined, "PHOTOGRAPHER_DISCRETION", "Photographer discretion", undefined, undefined, undefined, {
    "it-it": "Discrezione del fotografo",
    "en-gb": "Photographer discretion",
  }),
  new SelectItem(undefined, "CC_BY_NC_ND", "CC BY-NC-ND"),
  new SelectItem(undefined, "CC_BY_NC", "CC BY-NC"),
  new SelectItem(undefined, "CC_BY_ND", "CC BY-ND"),
  new SelectItem(undefined, "CC_BY", "CC BY"),
  new SelectItem(undefined, "PUBLIC_DOMAIN", "Public domain", undefined, undefined, undefined, {
    "it-it": "Pubblico dominio",
    "en-gb": "Public domain",
  }),
  new SelectItem(undefined, "ALL_RIGHTS_RESERVED", "All rights reserved", undefined, undefined, undefined, {
