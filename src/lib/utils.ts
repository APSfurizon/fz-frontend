...
export function copyContent(e: HTMLElement | HTMLInputElement) {
  if (e.textContent) {
    void navigator.clipboard.writeText(e.textContent);
    return true;
  } else if ("value" in e) {
...
export function firstOrUndefined(a: any[] | undefined): any {
...
function toTimeZoneWallClock(date: Date, timeZone: string): Date {
  ...
  const values = formatter.formatToParts(date).reduce<Record<string, number>>((acc, part) => {
