...
export interface AutoInputChangedParams {
  ...
  newValues?: AutoInputSearchResult[];
...
export class AutoInputFilter {
  ...
  applyFilter(data: AutoInputSearchResult): boolean {
    return (
      (data.code !== undefined && this.filteredCodes.includes(data.code.trim())) ||
...
export function filterLoaded(
  ...
  return results.filter(
    (result) => (filterIn?.applyFilter(result) ?? true) && !(filterOut?.applyFilter(result) ?? false)
  );
}
...
export interface AutoInputManager {
  // TODO: additionalValues overhaul
  ...
  /**
   * To be used with requiresParam in autoinput, enables the component whenever the parameter is initialized and there
   * are search results that are selectable
   * @param additionalValues  additional values to be used as search params
   */
