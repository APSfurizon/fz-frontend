...
export class InputEntity {
  ...
  public getDescription(): string {
    return this.description ?? this.id?.toString() ?? this.code ?? "";
  }
  constructor(
    id?: number,
    code?: string,
...
export const inputEntityIdExtractor = (entity: InputEntity) => {
  if (entity && !entity.id) throw new Error(`Invalid id on entity ${JSON.stringify(entity)}`);
  return entity?.id;
};

export const inputEntityCodeExtractor = (entity: InputEntity) => {
  if (entity && !entity.code) throw new Error(`Invalid code on entity ${JSON.stringify(entity)}`);
  return entity?.code;
};
