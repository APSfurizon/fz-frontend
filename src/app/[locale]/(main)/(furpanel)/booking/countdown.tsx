...
import { BookingOrderUiData, ShopLinkApiAction, ShopLinkResponse } from "@/lib/api/booking";
...
export default function Countdown({data}: Readonly<{data?: BookingOrderUiData}>) {
    ...
    {!isOpen && !data.hasOrder && (
        <NoticeBox title={t("furpanel.booking.messages.countdown.title")} theme={NoticeTheme.FAQ}>
            {t("furpanel.booking.messages.countdown.description", {openingDate: formatter.dateTime(data.bookingStartDate)})}
        </NoticeBox>
    )}
    <div className={`countdown-container rounded-s ${data.hasOrder ? "minimized" : ""}`}
        ...
        <img className="event-logo" alt={t("furpanel.booking.event_logo")} src={EVENT_LOGO} ></img>
        {/* Countdown view */}
        ...
        : data && !data.hasOrder && <div className="action-container">
            <Button className="action-button book-now" busy={actionLoading} disabled={data?.shouldUpdateInfo} onClick={requestShopLink}>
                <div className="vertical-list flex-vertical-center">
                    <span className="title large">{data?.shouldUpdateInfo ? <Icon style={{marginRight: ".2em"}} iconName={ICONS.LOCK}></Icon> : <></>}{t("furpanel.booking.book_now")}</span>
