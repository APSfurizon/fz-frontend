...
export default function SecurityHazardousRegisterPage() {
  ...
  const LIVELLO_LABEL: Record<string, string> = {
    basso: t("furpanel.admin.security_management.hazard.low"),
    medio: t("furpanel.admin.security_management.hazard.medium"),
    alto: t("furpanel.admin.security_management.hazard.high"),
    critico: t("furpanel.admin.security_management.hazard.critical"),
  };
  ...
  const openEdit = (h: SecurityHazard) => {
    setFTitolo(h.titolo);
    setFDescrizione(h.descrizione ?? "");
    setFLivello(h.livello);
    setFProprietarioNick(h.proprietario_nickname ?? "");
  ...
  const saveItem = () => {
    if (!fTitolo.trim()) {
      showModal(
        t("furpanel.admin.security_management.hazard.missing_dati"),
        <span>{t("furpanel.admin.security_management.hazard.missing_title")}</span>
      );
      return;
    }
  ...
  const filtered = hazards.filter((h) => {
    ...
    return [h.titolo, h.descrizione, h.proprietario_nickname].some((value) =>
      String(value ?? "")
        .toLowerCase()
    ...
    <div className="vertical-list gap-2mm">
      <div className="horizontal-list gap-2mm flex-vertical-center" style={{ flexWrap: "wrap", alignItems: "center" }}>
        ...
        <button
          ...
          onClick={() => setFilterLivello(null)}
          style={!filterLivello ? { background: "var(--button-background-active)" } : {}}
        >
          <span className="title normal">
            {t("furpanel.admin.security_management.hazard.all")} ({hazards.length})
          </span>
        </button>
        {LIVELLI.map((l) => {
          ...
          return (
            <button
              key={l}
              className="button rounded-m"
              onClick={() => setFilterLivello(filterLivello === l ? null : l)}
              style={
                filterLivello === l
                  ? { background: LIVELLO_COLOR[l], borderColor: LIVELLO_COLOR[l], color: "#fff" }
                  : {}
              }
            >
              <span className="title normal">
                {LIVELLO_LABEL[l]} ({count})
              </span>
            </button>
          );
        })}
      ...
      <div className="vertical-list gap-2mm table-container title rounded-m">
        {filtered.length === 0 && (
          <span className="title normal color-subtitle">
            {t("furpanel.admin.security_management.hazard.no_reports")}
          </span>
        )}
        {filtered.map((h) => (
          <div
            ...
            style={{
              ...
              borderLeft: `4px solid ${LIVELLO_COLOR[h.livello]}`,
            }}
            onClick={() => {
              setSelected(h);
              setView("detail");
          ...
          >
            <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center" }}>
              <div
                className="horizontal-list gap-2mm flex-vertical-center"
                style={{ marginBottom: 4, flexWrap: "wrap" }}
              >
                <span
                  style={{
                    background: LIVELLO_COLOR[h.livello],
                    color: "#fff",
                    fontWeight: 700,
                    fontSize: 11,
                    padding: "2px 8px",
              ...
              {h.descrizione && (
                <span className="title small color-subtitle" style={{ display: "block", marginTop: 2 }}>
                  {h.descrizione}
                </span>
              )}
              {h.trovato_da && (
                <span className="title small color-subtitle" style={{ display: "block", marginTop: 8 }}>
                  <Icon icon="PERSON" className="medium" /> {h.trovato_da}
                </span>
              )}
            </div>
            <div
  ...
  const renderForm = () => (
    <div className="vertical-list gap-3mm">
      <span className="title large" style={{ marginBottom: 8 }}>
        {isEdit
          ? t("furpanel.admin.security_management.hazard.edit_title")
          : t("furpanel.admin.security_management.hazard.new_title")}
      </span>
      <FpInput
        required
        label={t("furpanel.admin.security_management.hazard.title")}
        initialValue={fTitolo}
        onChange={(e) => setFTitolo(e.target.value ?? "")}
        placeholder={t("furpanel.admin.security_management.hazard.title_placeholder")}
      />
      <FpInput
        label={t("furpanel.admin.security_management.hazard.description")}
        initialValue={fDescrizione}
        onChange={(e) => setFDescrizione(e.target.value ?? "")}
        placeholder={t("furpanel.admin.security_management.hazard.description_placeholder")}
      />
      <FpInput
        required
        label={t("furpanel.admin.security_management.hazard.owner_nickname")}
        initialValue={fProprietarioNick}
        onChange={(e) => setFProprietarioNick(e.target.value ?? "")}
        placeholder={t("furpanel.admin.security_management.hazard.owner_nickname_placeholder")}
      />
      <FpInput
        required
        label={t("furpanel.admin.security_management.hazard.owner_id")}
        initialValue={fProprietarioId}
        onChange={(e) => setFProprietarioId(e.target.value ?? "")}
        placeholder={t("furpanel.admin.security_management.hazard.owner_id_placeholder")}
      />
      <div className="vertical-list gap-2mm">
      ...
      <div style={{ marginTop: 6 }}>
        <span className="title small" style={{ display: "block", marginBottom: 6 }}>
          {t("furpanel.admin.security_management.hazard.level")}
        </span>
        <div className="horizontal-list gap-2mm" style={{ flexWrap: "wrap" }}>
          {LIVELLI.map((l) => (
            <button
              key={l}
              className="button rounded-m"
              onClick={() => setFLivello(l)}
              style={
                fLivello === l ? { background: LIVELLO_COLOR[l], borderColor: LIVELLO_COLOR[l], color: "#fff" } : {}
              }
            >
              {LIVELLO_LABEL[l]}
            </button>
      ...
      <div className="horizontal-list gap-2mm" style={{ marginTop: 10 }}>
        ...
        >
          {t("furpanel.admin.security_management.hazard.cancel")}
        </FpButton>
        <FpButton icon="SAVE" busy={loading} onClick={saveItem}>
          {t("furpanel.admin.security_management.hazard.save")}
        </FpButton>
      </div>
    </div>
  );
  ...
  const renderDetail = (h: SecurityHazard) => (
    <div className="vertical-list gap-3mm">
      <div
        className="horizontal-list gap-2mm flex-vertical-center security-asset-detail-header"
      ...
      <div className="vertical-list gap-2mm security-asset-detail-list">
        {[
          [t("furpanel.admin.security_management.hazard.description"), h.descrizione],
          [t("furpanel.admin.security_management.hazard.reported_by"), h.trovato_da],
          [t("furpanel.admin.security_management.hazard.owner_nickname"), h.proprietario_nickname],
          [t("furpanel.admin.security_management.hazard.owner_id"), h.proprietario_id],
          ...
          .map(([label, value]) => (
            <div
              key={label}
              className="horizontal-list gap-2mm security-asset-detail-row"
              style={{ borderBottom: "1px solid #ffffff15", paddingBottom: 6 }}
            >
              <span className="title small color-subtitle security-asset-detail-label" style={{ minWidth: 160 }}>
                {label}
              </span>
        ...
        {(h.foto?.length ?? 0) > 0 && (
          <div className="vertical-list gap-2mm">
            <span className="title small color-subtitle security-asset-photo-title">
              {t("furpanel.admin.security_management.hazard.photo")} ({h.foto!.length})
            </span>
      ...
      <div className="horizontal-list flex-vertical-center gap-4mm flex-wrap" style={{ marginBottom: 8 }}>
        ...
        {view === "list" && (
          <FpButton icon="ADD" onClick={openAdd}>
            {t("furpanel.admin.security_management.hazard.add_report")}
          </FpButton>
        )}
        {view === "detail" && selected && (
          <FpButton icon="EDIT" onClick={() => openEdit(selected)}>
            {t("furpanel.admin.security_management.hazard.edit")}
          </FpButton>
        )}
      </div>
