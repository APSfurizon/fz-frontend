...
import {
  SecurityAsset,
  SecurityAssetLog,
  GetSecurityAssetsApiAction,
  GetSecurityAssetLogsApiAction,
  CreateSecurityAssetApiAction,
  UpdateSecurityAssetApiAction,
...
export default function SecurityAssetManagerPage() {
  ...
  const STATO_LABEL: Record<string, string> = {
    ...
    in_uso: t("furpanel.admin.security_management.assets.state_in_use"),
    non_disponibile: t("furpanel.admin.security_management.assets.state_unavailable"),
  };
  ...
  const resetForm = () => {
    ...
    setFSerial("");
    setFNote("");
    setFStato("disponibile");
    setFFormImage(undefined);
  };
  ...
  const openEdit = (a: SecurityAsset) => {
    setFTag(a.tag);
    setFTipo(a.device_tipo);
    setFModello(a.device_modello);
    setFSerial(a.device_serial_number ?? "");
    setFNote(a.note_condizioni ?? "");
    setFStato(a.stato);
    setSelected(a);
    setIsEdit(true);
    setView("form");
  ...
  const saveTransfer = () => {
    ...
    body.append(
      "utilizzatore_precedente",
      selected.utilizzatore_attuale ?? t("furpanel.admin.security_management.assets.not_set")
    );
  ...
  const filtered = assets.filter((a) => {
    ...
    return [a.tag, a.device_tipo, a.device_modello, a.device_serial_number].some((value) =>
      String(value ?? "")
        .toLowerCase()
  ...
  const renderList = () => (
    <div className="vertical-list gap-2mm">
      ...
      <div className="horizontal-list gap-2mm flex-vertical-center" style={{ flexWrap: "wrap", alignItems: "center" }}>
        <div style={{ flex: "1 1 320px", minWidth: 240, maxWidth: 560 }}>
          <input
            ...
            placeholder={t("furpanel.admin.security_management.assets.search_placeholder")}
          />
        </div>
        <div className="spacer" />
        <button
          className="button rounded-m"
          onClick={() => setFilterStato(null)}
          style={!filterStato ? { background: "var(--button-background-active)" } : {}}
        >
          <span className="title normal">
            {t("furpanel.admin.security_management.hazard.all")} ({assets.length})
          </span>
        </button>
        {STATI.map((s) => (
          <button
            key={s}
            className="button rounded-m"
            onClick={() => setFilterStato(filterStato === s ? null : s)}
      ...
      <div className="vertical-list gap-2mm table-container title rounded-m">
        {filtered.length === 0 && (
          <span className="title normal color-subtitle">{t("furpanel.admin.security_management.assets.no_items")}</span>
        )}
        {filtered.map((a) => (
          <div
            key={a.data}
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 4, flex: 1, minWidth: 0 }}>
              ...
              <span className="title normal" style={{ fontWeight: 700 }}>
                {[a.device_tipo, a.device_modello].filter(Boolean).join(" — ")}
              </span>
              <span className="title small color-subtitle">S/N: {a.device_serial_number}</span>
              {a.utilizzatore_attuale && (
                <span className="title small color-subtitle">
                  <Icon icon="PERSON" className="medium" /> {a.utilizzatore_attuale}
                </span>
              )}
            </div>
            ...
            <div
              style={{
                ...
                flexShrink: 0,
                minWidth: SECURITY_LIST_MEDIA_SLOT_WIDTH,
              }}
            >
              <span className="title small" style={{ color: STATO_COLOR[a.stato], fontWeight: 700 }}>
                {STATO_LABEL[a.stato]}
              </span>
              <div
                style={{
              ...
              >
                {(a.foto?.length ?? 0) > 1 && (
                  <span
                    style={{ background: "#1f6feb", color: "#fff", padding: "3px 8px", borderRadius: 8, fontSize: 12 }}
                  >
                    <Icon icon="PHOTO_CAMERA" className="small" /> {a.foto!.length}
                  </span>
                )}
                {(a.foto?.length ?? 0) > 0 ? (
                  <div onClick={(e) => e.stopPropagation()}>
                    <ImagePreviewModal
                      ...
                      alt={`${a.tag || t("furpanel.admin.security_management.assets.asset")} - preview`}
                      thumbSize={SECURITY_LIST_PREVIEW_SIZE}
                      title={`${a.tag || t("furpanel.admin.security_management.assets.asset")} - preview`}
                    />
                  </div>
                ) : (
                  <div
                    style={{
                      width: SECURITY_LIST_PREVIEW_SIZE,
                      height: SECURITY_LIST_PREVIEW_SIZE,
                      borderRadius: 8,
                      background: "#1f2b3a",
  ...
  const renderForm = () => (
    <div className="vertical-list gap-3mm">
      <span className="title large" style={{ marginBottom: 8 }}>
        {isEdit
          ? t("furpanel.admin.security_management.assets.edit_title")
          : t("furpanel.admin.security_management.assets.new_title")}
      </span>
      <FpInput
        label={t("furpanel.admin.security_management.assets.tag_label")}
        initialValue={fTag}
        onChange={(e) => setFTag(e.target.value ?? "")}
        placeholder={t("furpanel.admin.security_management.assets.tag_placeholder")}
      />
      <FpInput
        label={t("furpanel.admin.security_management.assets.type_label")}
        initialValue={fTipo}
        onChange={(e) => setFTipo(e.target.value ?? "")}
        placeholder={t("furpanel.admin.security_management.assets.type_placeholder")}
      />
      <FpInput
        label={t("furpanel.admin.security_management.assets.model_label")}
        initialValue={fModello}
        onChange={(e) => setFModello(e.target.value ?? "")}
        placeholder={t("furpanel.admin.security_management.assets.model_placeholder")}
      />
      <FpInput
        label={t("furpanel.admin.security_management.assets.serial_label")}
        initialValue={fSerial}
        onChange={(e) => setFSerial(e.target.value ?? "")}
        placeholder={t("furpanel.admin.security_management.assets.serial_placeholder")}
      />
      <FpInput
        label={t("furpanel.admin.security_management.assets.notes_label")}
        initialValue={fNote}
        onChange={(e) => setFNote(e.target.value ?? "")}
        placeholder={t("furpanel.admin.security_management.assets.notes_placeholder")}
      />
      <div className="vertical-list gap-2mm">
      ...
      <div style={{ marginTop: 6 }}>
        <span className="title small" style={{ display: "block", marginBottom: 6 }}>
          {t("furpanel.admin.security_management.assets.state_label")}
        </span>
      ...
      <div className="horizontal-list gap-2mm" style={{ marginTop: 10 }}>
        ...
        >
          {t("furpanel.admin.security_management.assets.cancel")}
        </FpButton>
        <FpButton icon="SAVE" busy={loading} onClick={saveItem}>
          {t("furpanel.admin.security_management.assets.save")}
        </FpButton>
      </div>
    </div>
  );
  ...
  const renderDetail = (a: SecurityAsset) => (
    <div className="vertical-list gap-3mm">
      <div className="horizontal-list gap-2mm flex-vertical-center security-asset-detail-header">
        <span className="title large security-asset-detail-title">
          {[a.tag, a.device_modello].filter(Boolean).join(" — ") ||
            t("furpanel.admin.security_management.assets.detail_fallback")}
        </span>
        <span
          className="security-asset-state-badge"
          style={{ ...SECURITY_BADGE_STYLE, background: STATO_COLOR[a.stato] }}
        >
          {STATO_LABEL[a.stato]}
        </span>
      </div>
      <div className="vertical-list gap-2mm security-asset-detail-list">
        {[
          [t("furpanel.admin.security_management.assets.tag_label"), a.tag],
          [t("furpanel.admin.security_management.assets.type_label"), a.device_tipo],
          [t("furpanel.admin.security_management.assets.model_label"), a.device_modello],
          [t("furpanel.admin.security_management.assets.serial_label"), a.device_serial_number],
          ...
          [
            t("furpanel.admin.security_management.assets.data_modifica_label"),
            a.data_modifica ? new Date(a.data_modifica).toLocaleString() : undefined,
          ],
        ...
        {(a.foto?.length ?? 0) > 0 && (
          <div className="vertical-list gap-2mm">
            <span className="title small color-subtitle security-asset-photo-title">
              {t("furpanel.admin.security_management.assets.photo")} ({a.foto!.length})
            </span>
      ...
      <div className="horizontal-list gap-2mm security-asset-detail-actions">
        <FpButton
          className="security-asset-transfer-btn"
          icon="SYNC"
          onClick={() => {
            setTUtilizzatore("");
            setTNote("");
            setTStato("disponibile");
            setTransferOpen(true);
          }}
        >
          {t("furpanel.admin.security_management.assets.transfer")}
        </FpButton>
      </div>
    </div>
  );
  ...
  const renderLogs = () => (
    <div className="vertical-list gap-2mm">
      <span className="title large">
        {t("furpanel.admin.security_management.assets.logs_title")}: {selected?.tag} - {selected?.device_modello}
      </span>
      {logsLoading && <LoadingPanel />}
      {!logsLoading && logs.length === 0 && (
        <span className="title normal color-subtitle">{t("furpanel.admin.security_management.assets.no_logs")}</span>
      )}
      {logs.map((l, idx) => (
        <div key={l.id ?? idx} className="stretch-page rounded-m security-asset-log-card">
          <div className="horizontal-list gap-2mm flex-vertical-center">
            <span
              style={{
                ...SECURITY_BADGE_STYLE,
                background: STATO_COLOR[l.stato],
                color: "#fff",
                padding: "4px 14px",
                borderRadius: 8,
                fontWeight: 700,
            ...
            <span className="title small color-subtitle" style={{ marginLeft: "auto" }}>
              {l.data_aggiornamento ? new Date(l.data_aggiornamento).toLocaleString() : ""}
            </span>
          </div>
          <div className="horizontal-list gap-2mm flex-vertical-center">
            <span className="title small">
              {t("furpanel.admin.security_management.assets.user_label")}: {l.utilizzatore ?? ""}
            </span>
            <span
              style={{
                background: "#2d3f54",
            ...
            >
              {l.azione} by {l.modificato_da}
            </span>
          </div>
          {l.note && (
            <span className="title small color-subtitle">
              {t("furpanel.admin.security_management.assets.notes_label")}: {l.note}
            </span>
          )}
        </div>
      ))}
  ...
  return (
    <div className="stretch-page compact-main">
      <div className="horizontal-list flex-vertical-center gap-4mm flex-wrap" style={{ marginBottom: 8 }}>
        ...
        <div className="spacer" />
        {view === "list" && (
          <FpButton icon="ADD" onClick={openAdd}>
            {t("furpanel.admin.security_management.assets.add")}
          </FpButton>
        )}
        {view === "detail" && selected && (
          <>
            <FpButton icon="EDIT" onClick={() => openEdit(selected)}>
              {t("furpanel.admin.security_management.assets.edit")}
            </FpButton>
            <FpButton icon="FIND_IN_PAGE" onClick={() => loadLogs(selected)}>
              {t("furpanel.admin.security_management.assets.view_logs")}
            </FpButton>
          </>
        )}
      </div>
      ...
      {transferOpen && selected && (
        <Modal
          ...
          busy={loading}
          title={t("furpanel.admin.security_management.assets.transfer_modal_title")}
          icon="SYNC"
          style={{ width: "min(92vw, 480px)" }}
        >
          <div className="vertical-list gap-3mm" style={{ padding: "1em" }}>
            <FpInput
              label={t("furpanel.admin.security_management.assets.new_user_label")}
              initialValue={tUtilizzatore}
              onChange={(e) => setTUtilizzatore(e.target.value ?? "")}
              placeholder={t("furpanel.admin.security_management.assets.new_user_placeholder")}
            />
            <FpInput
              label={t("furpanel.admin.security_management.assets.notes_label")}
              initialValue={tNote}
              ...
              <div className="horizontal-list gap-2mm" style={{ flexWrap: "wrap" }}>
                {STATI.map((s) => (
                  <button
                    ...
                    onClick={() => setTStato(s)}
                    style={
                      tStato === s ? { background: STATO_COLOR[s], borderColor: STATO_COLOR[s], color: "#fff" } : {}
                    }
                  >
                    {STATO_LABEL[s]}
                  </button>
