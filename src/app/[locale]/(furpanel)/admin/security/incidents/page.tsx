...
export default function SecurityIncidentsPage() {
  ...
  const loadReports = (history = showHistory) => {
    ...
    return runRequest({ action: new GetSecurityIncidentsApiAction(), searchParams: params })
      .then((response) => {
        ...
        if (history) {
          setHistoryReports(
            [...(mappedReports ?? []), ...(mappedDisabled ?? [])].sort((a, b) => (b.data ?? 0) - (a.data ?? 0))
          );
        } else {
  ...
  const saveIncident = () => {
    ...
    if (message.length < 3) {
      showModal(
        t("furpanel.admin.security_management.incidents.missing_data_title"),
        <span>{t("furpanel.admin.security_management.incidents.missing_data_description")}</span>
      );
      return;
    }
  ...
  const renderReportCard = (item: SecurityIncident, disabled = false) => {
    ...
    return (
      <div
        key={`${item.data}_${item.fileName ?? "report"}`}
        className="rounded-m"
        style={{
          padding: "0.75em",
          margin: 0,
      ...
      >
        <div style={{ flex: 1 }}>
          ...
          <span className="title normal" style={{ fontWeight: 700, display: "block" }}>
            {item.titolo || t("furpanel.admin.security_management.incidents.untitled")}
          </span>
          {item.from && (
        ...
        <div className="vertical-list gap-2mm" style={{ alignItems: "flex-end", flexShrink: 0 }}>
          <span className="title small color-subtitle" style={{ whiteSpace: "nowrap" }}>
            {t("furpanel.admin.security_management.incidents.messages")} ({messageCount})
          </span>
          <div className="horizontal-list gap-2mm" style={{ justifyContent: "flex-end", flexWrap: "wrap" }}>
            {item.importante &&
              !isHistory &&
              renderBadge(t("furpanel.admin.security_management.incidents.important"), "#f39c12")}
            {item.sospeso &&
              !isHistory &&
              renderBadge(t("furpanel.admin.security_management.incidents.suspended"), "#7f8c8d")}
  ...
  const renderReportGroup = (title: string, emptyText: string, list: SecurityIncident[], disabled = false) => (
  ...
  const renderCreate = () => (
    <div className="vertical-list gap-3mm">
      ...
      <FpInput
        label={t("furpanel.admin.security_management.incidents.title")}
        initialValue={newTitle}
        onChange={(e) => setNewTitle(e.target.value ?? "")}
        placeholder={t("furpanel.admin.security_management.incidents.title_placeholder")}
      />
      <FpInput
        label={t("furpanel.admin.security_management.incidents.people_involved")}
        initialValue={newPeople}
      ...
      <div className="horizontal-list gap-2mm">
        ...
        >
          {t("furpanel.admin.security_management.incidents.cancel")}
        </FpButton>
        <FpButton icon="SEND" busy={loading} onClick={saveIncident}>
          {t("furpanel.admin.security_management.incidents.send")}
        </FpButton>
      </div>
    </div>
  );
      ...
      <div className="rounded-m vertical-list gap-3mm" style={{ width: "100%", margin: 0, padding: "0.75em" }}>
        <div className="vertical-list gap-2mm">
          <div className="horizontal-list gap-2mm flex-vertical-center" style={{ flexWrap: "wrap" }}>
            <span className="title large" style={{ flex: 1, minWidth: 0 }}>
              {item.titolo || t("furpanel.admin.security_management.incidents.report")}
            </span>
            <div
            ...
            >
              ...
              {item.sospeso &&
                ...
                renderBadge(t("furpanel.admin.security_management.incidents.suspended"), "#7f8c8d")}
              {item.folder === "cronologia" &&
                renderBadge(t("furpanel.admin.security_management.incidents.history"), "#8e44ad")}
            </div>
          </div>
          <span className="title small color-subtitle">
            {t("furpanel.admin.security_management.incidents.reported_by")}: {item.from || "-"}
          </span>
          <span className="title small color-subtitle" style={{ marginBottom: "0.5em" }}>
            {item.textData}
          </span>
        </div>
        ...
        {people.length > 0 && (
          ...
          >
            <span className="title small color-subtitle">
              {t("furpanel.admin.security_management.incidents.people_involved")}:
            </span>
            <div className="horizontal-list gap-2mm" style={{ flexWrap: "wrap" }}>
              {people.map((name, idx) => (
                <span key={`${name}_${idx}`} style={{ ...INCIDENT_BADGE_STYLE, background: "#132033", color: "#fff" }}>
                  {name}
                </span>
              ))}
            </div>
        ...
        <div className="vertical-list gap-2mm">
          {(item.messaggi ?? []).map((message, idx) => {
            const isCurrentUserMessage =
              (message.from ?? "").trim().toLowerCase() === (currentUserName ?? "").trim().toLowerCase();
            return (
              <div
              ...
              >
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div className="horizontal-list gap-2mm flex-vertical-center" style={{ flexWrap: "wrap" }}>
                    <span className="title small" style={{ fontWeight: 700 }}>
                      {message.from || "-"}
                    </span>
                  ...
                  <div style={{ flexShrink: 0, marginLeft: "auto" }}>
                    <ImagePreviewModal
                      ...
                      title={(
                        ...
                        item.titolo ||
                        t("furpanel.admin.security_management.incidents.photo_preview")
                      ).slice(0, 48)}
                    />
                  </div>
                )}
          ...
          {(item.messaggi?.length ?? 0) === 0 && (
            <span className="title small color-subtitle">
              {t("furpanel.admin.security_management.incidents.no_messages")}
            </span>
          )}
        </div>
          ...
          <div className="vertical-list gap-2mm" style={{ marginTop: "0.8em" }}>
            <div className="horizontal-list gap-2mm">
              ...
              <div className="vertical-list gap-2mm" style={{ flex: 1 }}>
                <span className="title small color-subtitle">
                  {t("furpanel.admin.security_management.incidents.add_message")}
                </span>
                <textarea
                  value={replyMessage}
                ...
                <div className="horizontal-list gap-2mm" style={{ justifyContent: "flex-end" }}>
                  <FpButton icon="SEND" busy={loading} disabled={!replyMessage.trim()} onClick={sendReply}>
                    {t("furpanel.admin.security_management.incidents.send_message")}
                  </FpButton>
                </div>
              </div>
            </div>
  ...
  return (
    <div className="stretch-page compact-main">
      <div className="horizontal-list flex-vertical-center gap-4mm flex-wrap" style={{ marginBottom: 8 }}>
        ...
        {view === "list" && (
          <FpButton onClick={refreshList} busy={refreshing}>
            {t("furpanel.admin.security_management.incidents.refresh")}
          </FpButton>
        )}
        ...
        {view === "list" && (
          <>
            {!showHistory && (
              ...
              >
                {t("furpanel.admin.security_management.incidents.add")}
              </FpButton>
            )}
            <FpButton onClick={() => setShowHistory((prev) => !prev)}>
              {showHistory
                ? t("furpanel.admin.security_management.incidents.reports")
                : t("furpanel.admin.security_management.incidents.history")}
            </FpButton>
          </>
        )}

        {view === "detail" && selected && (
          <FpButton icon="EDIT" onClick={openEditMeta}>
            {t("furpanel.admin.security_management.incidents.edit")}
          </FpButton>
        )}
      </div>
      ...
      <Modal
        ...
        title={t("furpanel.admin.security_management.incidents.edit_report")}
        icon="EDIT"
      ...
      >
        <div className="vertical-list gap-3mm" style={{ padding: "0.9em" }}>
          ...
          <label
            className="horizontal-list gap-2mm flex-vertical-center"
            style={{ width: "fit-content", marginTop: "0.2em" }}
          >
            <input
              type="checkbox"
              checked={detailImportant}
              onChange={(e) => {
          ...
          <div className="horizontal-list gap-2mm" style={{ marginTop: "0.5em" }}>
            <FpButton onClick={() => setDetailsModalOpen(false)}>
              {t("furpanel.admin.security_management.incidents.cancel")}
            </FpButton>
            <FpButton icon="CHECK" busy={loading} onClick={saveMeta}>
              {t("furpanel.admin.security_management.incidents.update")}
            </FpButton>
          </div>
        </div>
      </Modal>
