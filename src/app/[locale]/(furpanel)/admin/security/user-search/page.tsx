...
function SectionCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    ...
    >
      ...
      <div style={{ padding: "0.8em" }}>{children}</div>
    </div>
  );
}
  ...
  const searchConfig: UserSearchConfig = useMemo(() => {
    switch (currentCriteria) {
      ...
      case SearchCriteria.ORDER_SERIAL:
        return {
          manager: new AutoInputUsersManager(
            new UserSearchByOrderSerialAction(),
      ...
      case SearchCriteria.MEMBERSHIP_CARD:
        return {
          manager: new AutoInputUsersManager(
            new UserSearchByMembershipNumberAction(),
        ...
        {userData && (
          <div className="vertical-list gap-3mm" key={"security_search"} id={"security_search"}>
            ...
            <SectionCard title={t("furpanel.admin.users.security.user_search.sections.user_data")}>
              <div
                className="horizontal-list gap-4mm flex-vertical-center"
                style={{ alignItems: "flex-start", marginBottom: "0.8em" }}
              >
                {userData.badgeData?.mainBadge?.propic?.mediaUrl ? (
                  <ImagePreviewModal
                    ...
                    title={
                      ...
                      t("furpanel.admin.users.security.user_search.alt.user")
                    }
                  />
                ) : (
                ...
                <div className="security-data-grid" style={{ flex: 1 }}>
                  <FpInput
                    label={t("furpanel.admin.users.accounts.view.badges.fursona_name")}
                    initialValue={userData.badgeData?.mainBadge?.fursonaName ?? ""}
                    readOnly
                  />
                  <FpInput
                    label={t("furpanel.admin.users.accounts.view.orders_table.order_code")}
                    initialValue={userData.currentOrder?.code ?? ""}
                    readOnly
                  />
                  <FpInput
                    label={t("furpanel.admin.users.accounts.view.badges.user_id")}
                    initialValue={
                      userData.badgeData?.mainBadge?.userId != null ? String(userData.badgeData.mainBadge.userId) : ""
                    }
                    readOnly
                  />
                  <FpInput
                    label={t("furpanel.admin.users.accounts.view.badges.locale")}
                    initialValue={userData.badgeData?.mainBadge?.locale ?? ""}
                    readOnly
                  />
                  <FpInput
            ...
            <SectionCard title={t("furpanel.admin.users.security.user_search.sections.user_info")}>
              <div className="security-data-grid-3">
                <FpInput
                  label={t("furpanel.admin.users.security.user_search.fields.name")}
                  initialValue={userData.firstName ?? ""}
                  readOnly
                />
                <FpInput
                  label={t("furpanel.admin.users.security.user_search.fields.surname")}
                  initialValue={userData.lastName ?? ""}
                  readOnly
                />
                <FpInput
                  label={t("furpanel.admin.users.security.user_search.fields.mail")}
                  initialValue={userData.email ?? ""}
                  readOnly
                />
                <FpInput
                  label={t("furpanel.admin.users.security.user_search.fields.sex")}
                  initialValue={userData.sex ? (sexOptions[userData.sex] ?? userData.sex) : ""}
                  readOnly
                />
                <FpInput
                  label={t("furpanel.admin.users.security.user_search.fields.gender")}
                  initialValue={userData.gender ? (genderOptions[userData.gender] ?? userData.gender) : ""}
                  readOnly
                />
                <FpInput
                  label={t("furpanel.admin.users.security.user_search.fields.birthday")}
                  initialValue={userData.birthday ?? ""}
                  readOnly
                />
                <FpInput
                  label={t("furpanel.admin.users.security.user_search.fields.allergies")}
                  initialValue={userData.allergies ?? ""}
                  readOnly
                />
                <FpInput
                  label={t("furpanel.admin.users.security.user_search.fields.telegram")}
                  initialValue={userData.telegramUsername ?? ""}
                  readOnly
                />
                <FpInput
                  label={t("furpanel.admin.users.security.user_search.fields.phone")}
                  initialValue={`${userData.prefixPhoneNumber || ""} ${userData.phoneNumber || ""}`.trim()}
                  readOnly
                />
              </div>
              <div className="horizontal-list gap-2mm" style={{ marginTop: "0.8em" }}>
                <FpButton icon="SEND" onClick={() => openTelegram(userData.telegramUsername ?? undefined)}>
                  Telegram
                </FpButton>
                <FpButton
                  onClick={() => openPhone(userData.prefixPhoneNumber ?? undefined, userData.phoneNumber ?? undefined)}
                >
                  {t("furpanel.admin.users.security.user_search.actions.call")}
                </FpButton>
              </div>
            ...
            {userData.currentMembershipCard && (
              <SectionCard title={t("furpanel.admin.users.accounts.view.membership_cards")}>
                <div className="security-data-grid">
                  <FpInput
                    label={t("furpanel.admin.users.accounts.view.cards_table.card_no")}
                    initialValue={userData.currentMembershipCard.cardNo ?? ""}
                    readOnly
                  />
                  <FpInput
                    label={t("furpanel.admin.users.accounts.view.cards_table.issue_year")}
                    initialValue={
                      userData.currentMembershipCard.issueYear != null
                        ? String(userData.currentMembershipCard.issueYear)
                        : ""
                    }
                    readOnly
                  />
                  <FpInput
                    label={t("furpanel.admin.users.accounts.view.cards_table.id_in_year")}
                    initialValue={
                      userData.currentMembershipCard.idInYear != null
                        ? String(userData.currentMembershipCard.idInYear)
                        : ""
                    }
                    readOnly
                  />
                  <FpInput
                    label={t("furpanel.admin.users.accounts.view.cards_table.aps_form")}
                    initialValue={
                      userData.currentMembershipCard.signedAt
                        ? t("furpanel.admin.users.accounts.view.cards_table.aps_form_signed")
                        : ""
                    }
                    readOnly
                  />
                  <FpInput
                    label={t("furpanel.admin.users.accounts.view.cards_table.sent_by_email")}
                    initialValue={
                      userData.currentMembershipCard.sentByEmail
                        ? t("furpanel.admin.users.security.user_search.values.yes")
                        : t("furpanel.admin.users.security.user_search.values.no")
                    }
                    readOnly
                  />
                </div>
            ...
            {userData.currentOrder && (
              <SectionCard title={t("furpanel.admin.users.accounts.view.orders")}>
                <div className="security-data-grid">
                  ...
                  />
                  <FpInput
                    label={t("furpanel.admin.users.accounts.view.orders_table.event_order_serial")}
                    initialValue={
                      userData.currentOrder.orderSerialInEvent != null
                        ? String(userData.currentOrder.orderSerialInEvent)
                        : ""
                    }
                    readOnly
                  />
                  <FpInput
                    label={t("furpanel.admin.users.accounts.view.orders_table.order_status")}
                    initialValue={userData.currentOrder.orderStatus ?? ""}
                    readOnly
                  />
                  <FpInput
                    label={t("furpanel.admin.users.accounts.view.orders_table.sponsorship_type")}
                    initialValue={userData.currentOrder.sponsorship ?? ""}
                    readOnly
                  />
                  <FpInput
                    label={t("furpanel.admin.users.security.user_search.fields.board")}
                    initialValue={userData.currentOrder.board ?? ""}
                    readOnly
                  />
                  <div style={{ gridColumn: "span 2" }}>
                    <FpInput
                      label={t("furpanel.admin.users.accounts.view.orders_table.daily_days")}
                      initialValue={
                        userData.currentOrder?.dailyDaysDates?.length
                          ? [...userData.currentOrder.dailyDaysDates].sort().join(", ")
                          : ""
                      }
                      readOnly
                    />
            ...
            <SectionCard title={t("furpanel.admin.users.security.user_search.sections.user_order_room")}>
              <div className="security-data-grid-3">
                <FpInput
                  label={t("furpanel.admin.users.security.user_search.fields.have_order")}
                  initialValue={
                    userData.currentRoomdata?.hasOrder
                      ? t("furpanel.admin.users.security.user_search.values.yes")
                      : t("furpanel.admin.users.security.user_search.values.no")
                  }
                  readOnly
                />
                <FpInput
                  label={t("furpanel.admin.users.security.user_search.fields.have_room")}
                  initialValue={
                    userData.currentRoomdata?.currentRoomInfo?.roomId
                      ? t("furpanel.admin.users.security.user_search.values.yes")
                      : t("furpanel.admin.users.security.user_search.values.no")
                  }
                  readOnly
                />
                <FpInput
                  label={t("furpanel.admin.users.security.user_search.fields.is_owner")}
                  initialValue={
                    roomOwnerUserId === userData.badgeData?.mainBadge?.userId
                      ? t("furpanel.admin.users.security.user_search.values.yes")
                      : t("furpanel.admin.users.security.user_search.values.no")
                  }
                  readOnly
                />
                <FpInput
                  label={t("furpanel.admin.users.security.user_search.fields.room_name")}
                  initialValue={userData.currentRoomdata?.currentRoomInfo?.roomName ?? ""}
                  readOnly
                />
                <FpInput
                  label="Hotel / Room Type"
                  initialValue={
                    userData.currentRoomdata?.currentRoomInfo?.roomData?.roomTypeNames?.["en"] ??
                    userData.currentRoomdata?.currentRoomInfo?.roomData?.roomInternalName ??
                    ""
                  }
                  readOnly
                />
                <FpInput
                  label={t("furpanel.admin.users.security.user_search.fields.room_capacity")}
                  initialValue={
                    userData.currentRoomdata?.currentRoomInfo?.roomData?.roomCapacity != null
                      ? String(userData.currentRoomdata.currentRoomInfo.roomData.roomCapacity)
                      : ""
                  }
                  readOnly
                />
                <FpInput
                  label={t("furpanel.admin.users.accounts.view.orders_table.extra_days")}
                  initialValue={
                    userData.currentRoomdata?.currentRoomInfo?.extraDays != null
                      ? String(userData.currentRoomdata.currentRoomInfo.extraDays)
                      : ""
                  }
                  readOnly
                />
              ...
              {(userData.currentRoomdata?.currentRoomInfo?.guests?.length ?? 0) > 0 && (
                <div style={{ marginTop: "0.8em" }}>
                  <span className="security-data-label">
                    {t("furpanel.admin.users.security.user_search.fields.guests")}
                  </span>
                  <div className="vertical-list gap-2mm" style={{ marginTop: "0.4em" }}>
                    {userData
                      ...
                      .map((g, idx) => {
                        ...
                        return (
                          <div
                            key={`${g.user?.userId || idx}`}
                            className="horizontal-list gap-3mm flex-vertical-center"
                            style={{
                              padding: "0.3em 0.5em",
                              borderRadius: 8,
                              background: isOwner ? "var(--highlight)22" : "transparent",
                              border: isOwner ? "1px solid var(--highlight)66" : "none",
                            }}
                          >
                            {g.user?.propic?.mediaUrl ? (
                              ...
                              <div
                                style={{ width: 36, height: 36, borderRadius: 8, background: "#2c3e50", flexShrink: 0 }}
                              />
                            )}
                            <span
                              className="security-data-value"
                              style={{ fontWeight: isOwner ? 700 : undefined, marginLeft: "0.5em" }}
                            >
                              {g.user?.fursonaName || "-"}
                              {isOwner && ` ${t("furpanel.admin.users.security.user_search.fields.owner_label")}`}
                            </span>
                            <span className="security-data-label" style={{ marginLeft: "auto" }}>
                              {g.orderStatus}
                            </span>
                            <button
                              type="button"
                  ...
                  <div className="vertical-list gap-2mm">
                    {(userData.badgeData?.fursuits ?? []).map((item, index) => (
                      ...
                      >
                        <div
                          className="horizontal-list gap-4mm flex-vertical-center"
                          style={{ alignItems: "flex-start", padding: "0.6em" }}
                        >
                          {item.fursuit?.propic?.mediaUrl ? (
                            ...
                            <div
                              style={{ width: 80, height: 80, borderRadius: 12, background: "#2c3e50", flexShrink: 0 }}
                            ></div>
                          )}
                          <div className="security-data-grid" style={{ flex: 1 }}>
                            <FpInput
                              label="ID"
                              initialValue={item.fursuit?.id != null ? String(item.fursuit.id) : ""}
                              readOnly
                            />
                            <FpInput
                              label={t("furpanel.admin.users.accounts.view.fursuit_table.name")}
                              initialValue={item.fursuit?.name ?? ""}
                              readOnly
                            />
                            <FpInput
                              label={t("furpanel.admin.users.accounts.view.fursuit_table.species")}
                              initialValue={item.fursuit?.species ?? ""}
                              readOnly
                            />
                            <FpInput
                              label={t("furpanel.admin.users.security.user_search.fields.bring_to_event")}
                              initialValue={
                                item.bringingToEvent
                                  ? t("furpanel.admin.users.security.user_search.values.yes")
                                  : t("furpanel.admin.users.security.user_search.values.no")
                              }
                              readOnly
