...
import { mapOrderStatusToStatusBox, qrCodeLogo, qrCodeOptions } from "@/lib/api/reservation";
...
export default function UserViewOrdersTable({
  ...
  const renderDailyDaysDates = (row: Row<FullOrder>) => {
    const dailyDays = row.original.dailyDaysDates
      .toSorted()
      .map((day) => formatter.dateTime(new Date(day), { dateStyle: "medium" }));
    return (
      <div className="vertical-list">
        <span className="title medium">{t("furpanel.admin.users.accounts.view.orders_table.daily_days")}</span>
        {dailyDays.map((d, i) => (
          <span className="monospace" key={i}>
            {d}
          </span>
        ))}
      </div>
  ...
  const orderColumns = useMemo(
    () => [
      ...
      orderColHelper.accessor(
        (itm) =>
          itm.roomCapacity > 0 ? `${itm.hotelInternalName} - ${itm.roomInternalName} (${itm.roomCapacity})` : undefined,
        {
          id: "roomType",
        ...
        cell: (props) => (
          <div className="horizontal-list gap-2mm">
            ...
            <FpButton
              ...
              onClick={() =>
                showModal(
                  t("furpanel.admin.users.accounts.view.orders_table.actions.view_qr.title"),
                  <div className="vertical-list">
                    <Canvas text={props.row.original.checkinSecret} options={qrCodeOptions} logo={qrCodeLogo} />
