...
export default function UserViewRooms({
  ...
  const getRoomType = (row: RoomInfoResponse) => {
    ...
    if (info.roomData.roomCapacity > 0) {
      return `${translate(info.roomData.roomTypeNames, locale)} ` + `(${info.roomData.roomCapacity})`;
    } else {
      return "";
    }
  };
  ...
  const [roomColumns] = useState([
    roomColHelper.accessor((itm) => translate(events[itm.currentRoomInfo.eventId].eventNames, locale), {
      id: "eventName",
    ...
    roomColHelper.display({
      ...
      cell: (props) =>
        !events[props.row.original.currentRoomInfo.eventId].current ? undefined : (
  ...
  return (
    <>
      ...
      <AddGuestModal
        ...
        reloadData={reloadData}
      />
      <RenameRoomModal roomInfo={currentRow} open={renameModalOpen} onClose={closeRenameModal} />
      <DeleteRoomModal roomInfo={currentRow} open={deleteModalOpen} onClose={closeDeleteModal} />
    </>
  );
}
