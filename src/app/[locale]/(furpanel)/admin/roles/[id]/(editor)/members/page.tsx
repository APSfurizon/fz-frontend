...
export default function RoleMembersEditor() {
  ...
  const addMember = () => {
    ...
    if (entity.users.find((usr) => usr.displayData.userId === toAdd.displayData.userId)) {
      showModal(
        t("furpanel.admin.users.security.roles.messages.duplicate_user.title"),
  ...
  return (
    <>
        ...
        <div className="table rounded-m">
          {entity?.users?.map((roleMember, mi) => (
            <div key={mi} className="row horizontal-list align-items-center gap-2mm flex-wrap">
              ...
              <div className="data">
                <Checkbox
                  initialValue={roleMember.tempRole}
      ...
      <Modal
        ...
        title={t("furpanel.admin.users.security.roles.input.add_member.label")}
        style={{ minWidth: "400px" }}
      >
        <DataForm
          action={new DummyFormAction()}
          shouldReset={!addMemberOpen}
          hideSave
        ...
        >
          <AutoInput
            ...
            filterOut={
              new AutoInputFilter(
                entity?.users.map((eu) => eu.displayData.userId),
                []
              )
            }
        ...
        <div className="horizontal-list gap-4mm">
          <FpButton type="button" className="danger" icon="CANCEL" onClick={() => setAddMemberOpen(false)}>
            {t("common.cancel")}
          </FpButton>
          <div className="spacer"></div>
          <FpButton
