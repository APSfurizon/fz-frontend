...
export default function QuotaViewer({
  isOpen,
  modalLoading,
  setModalLoading,
}: Readonly<{
  ...
  return (
    <>
      <div className="horizontal-list gap-4mm align-items-center">
        <NoticeBox theme={NoticeTheme.FAQ}>{t("furpanel.room.quota_viewer.description")}</NoticeBox>
        <div className="spacer"></div>
        <div>
          <FpButton icon="REFRESH" onClick={() => setRoomsData(null)} debounce={3000} busy={modalLoading}>
            {t("common.reload")}
          </FpButton>
        </div>
      </div>
