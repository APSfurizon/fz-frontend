...
import { loadScheduleActivityDetail } from "@/lib/api/schedule/main";
...
import "@/styles/misc/schedule/detail.css";
            ...
            <div className="schedule-detail-info-panel vertical-list gap-0">
              ...
              <div className="schedule-detail-header-top">
                <button
                  ...
                  onClick={() =>
                    router.push(selectedDayKey ? `/schedule?day=${encodeURIComponent(selectedDayKey)}` : "/schedule")
                  }
                  type="button"
              ...
              <div className="schedule-detail-info-list vertical-list gap-3mm">
                ...
                {activity.host && (
                  <InfoField icon="PERSON" label={t("schedule_detail.labels.host")} value={activity.host} />
                )}
                <InfoField icon="LOCATION_ON" label={t("schedule_detail.labels.location")} value={activity.location} />
                {activity.language && (
                  <InfoField icon="LANGUAGE" label={t("schedule_detail.labels.language")} value={activity.language} />
                )}
              </div>

              {/* Sponsor Tags */}
              <div className="schedule-detail-tags-section horizontal-list gap-2mm flex-wrap">
                {activity.ultra_sponsor && (
                  <span className="schedule-detail-tag tag-explorer">{t("schedule_detail.tags.explorer")}</span>
                )}
                {activity.super_sponsor && (
                  <span className="schedule-detail-tag tag-super">{t("schedule_detail.tags.super_sponsor")}</span>
                )}
                {activity.sponsor && (
                  <span className="schedule-detail-tag tag-sponsor">{t("schedule_detail.tags.sponsor")}</span>
                )}
                {activity.cancellato && (
                  <span className="schedule-detail-tag tag-canceled">{t("schedule_detail.tags.cancelled")}</span>
                )}
              </div>
            </div>
  ...
  return (
    <div className="schedule-detail-info-field horizontal-list gap-2mm">
      ...
      <div className="schedule-detail-info-content vertical-list">
        <span className="schedule-detail-info-value">
