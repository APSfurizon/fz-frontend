...
export default function ViewMediaModal(props: Readonly<ViewMediaModalProps>) {
    ...
    const isAdmin = useMemo(() => userDisplayRef.current?.permissions?.includes(Permissions.UPLOADS_CAN_MANAGE_UPLOADS),
    ...
    const ownsMedia = useMemo(() => userDisplayRef.current && fullMedia &&
        fullMedia?.photographer.userId === userDisplayRef.current.display.userId,
        [fullMedia, userDisplayRef.current]);

    const canDeleteMedia = useMemo(() => ownsMedia ||
        userDisplayRef.current?.permissions?.includes(Permissions.UPLOADS_CAN_FULLY_DELETE_UPLOADS),
    ...
    const deleteMedia = useCallback(() => {
        ...
        runRequest({
            ...
            err => showModal(t("common.error"), <ErrorMessage error={err} />, "ERROR")
        ).finally(() => {
            setLoading(false);
    ...
    return <>
                ...
                <div className={[
                    ...
                    {fullMedia && <div className="view-media-modal__panel-data">
                        ...
                        <ModalPanelData icon="LOCAL_ACTIVITY" text={translate(fullMedia.event.eventNames, locale)}>
                            {isAdmin && <>
                                &nbsp;
                                {fullMedia.selected && <span className="descriptive bold">({t("misc.gallery.modal.actions.admin.selected")})&nbsp;</span>}
                                <FpButton style={{ display: "inline" }}
                                    busy={loading}
                                    onClick={toggleBannerSelection}
                                    icon={fullMedia.selected ? "KEEP_OFF" : "KEEP"}
                                    title={fullMedia.selected
                        ...
                        {fullMedia.videoMetadata && <>
                            <h4 className="view-media-modal__panel-header title">{t("misc.gallery.modal.information_panel.video.title")}</h4>
                            <div className="vertical-list">
                                <ModalPanelData icon="24FPS_SELECT" text={fullMedia.videoMetadata.framerate} />
                                {duration && <ModalPanelData icon="AV_TIMER" text={t("misc.gallery.modal.information_panel.video.duration", {
                                    hours: duration[1],
                                    minutes: duration[2],
                                    seconds: duration[3],
        ...
        <Modal className="view-media-modal__delete-modal"
            zIndex={501}
            title={t("common.CRUD.delete")}
            icon="DELETE"
            ...
            <span className="title average">
                {t("misc.gallery.modal.actions.delete.message")}
            </span>
