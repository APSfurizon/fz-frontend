...
export default function GalleryEvents() {
    ...
    const sortedEvents = useMemo(() => Array.from(events.entries())
        .sort(([_1, a], [_2, b]) => {
            return new Date(b.event.correctDateFrom).getTime() - new Date(a.event.correctDateFrom).getTime();
        }),
        [events])

    return !currentFilter?.eventId && <div className="gallery-events__container gap-4mm flex-wrap">
        {sortedEvents.map(([id, value]) => <EventCard key={id} event={value} onClick={e => setFilter({ eventId: e.event.id })} />)}
    </div>
}