      ...
      <div className="attendee-content rounded-s">
        ...
        <div className="attendee-data">
          ...
          <div className="attendee-extra">
            {props.extraDays && props.extraDays !== ExtraDays.NONE && (
              <StatusBox>{t(`furpanel.booking.items.extra_days_${props.extraDays}`)}</StatusBox>
            )}
          </div>
        </div>
        <div className="attendee-badges">
          {props.data.locale && <span className="flag medium">{getFlagEmoji(props.data.locale?.toLowerCase())}</span>}
          {props.data.sponsorship !== SponsorType.NONE && (
            <Icon icon="WORKSPACE_PREMIUM" title={t(`common.sponsorships.${props.data.sponsorship.toLowerCase()}`)} />
          )}
        </div>
      </div>
    </div>
