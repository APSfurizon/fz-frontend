...
import {
  ...
  FursuitCountResponse,
  NoseCountApiAction,
  NoseCountResponse,
  SponsorCountApiAction,
...
export default function NosecountPage({ params }: { params: Promise<{ slug: string[] }> }) {
  ...
  useEffect(() => {
    if (!roomsData) return;
    const hotelGuestCount = roomsData.hotels
      .map((hotel) => hotel.roomTypes.map((roomType) => roomType.rooms.map((room) => room.guests)))
      .flat(3).length;

    const dailyFursCount = new Set(
      Object.keys(roomsData.dailyFurs)
        .map((day) => roomsData.dailyFurs[day].map((user) => user.userId))
        .flat()
    ).size;
      ...
      {mode == CountViewMode.NORMAL && roomsData && (
        <>
          ...
          <div className="vertical-list gap-4mm">
            {roomsData.hotels.map((hotel, hi) => (
              <div key={hi} className="hotel-container">
                ...
                {/* Room type */}
                {hotel.roomTypes.map((roomType, rti) => (
                  <div key={`rt${hi}-${rti}`} className="room-type">
                    ...
                    <div className="room-type-container">
                      {/* Room */}
                      {roomType.rooms.map((room, ri) => (
                        <div key={`ri-${hi}-${rti}-${ri}`} className="room-container">
                          <div
                            key={`rh${hi}-${rti}-${ri}`}
                          ...
                          >
                            ...
                            {room.roomExtraDays && room.roomExtraDays != ExtraDays.NONE && (
                              <StatusBox>{t(`furpanel.booking.items.extra_days_${room.roomExtraDays}`)}</StatusBox>
                            )}
                          </div>
                          <div key={`rgl-${hi}-${rti}-${ri}`} className="room-guests gap-2mm">
                            {room.guests.map((guest, ui) => (
                              <NosecountAttendee key={`rgl-${hi}-${rti}-${ri}-${ui}`} data={guest.user} />
                            ))}
                          </div>
                        </div>
                      ))}
          ...
          <div className="user-list gap-4mm">
            {roomsData.roomlessFurs.map((data, index) => (
              <NosecountAttendee key={`rf-${index}`} data={data.user} extraDays={data.extraDays} />
            ))}
          </div>
          ...
          {Object.keys(roomsData.dailyFurs).length > 0 && (
            <>
              ...
              {Object.keys(roomsData.dailyFurs).map((day, di) => (
                <div className="daily-day" key={di}>
