    ...
    <>
      <div className="room-invite vertical-list gap-4mm rounded-s">
        ...
        <div className="room-guests horizontal-list gap-4mm justify-content-center flex-space-evenly flex-wrap">
          {inviteData.room.guests
            .filter((usr) => usr.roomGuest.confirmed)
            .map((usr, key) => (
              <div key={key} className="guest-container vertical-list gap-2mm">
                <UserPicture key={key} size={64} userData={usr.user} showNickname showFlag />
                {inviteData.room.roomOwner.userId === usr.user.userId && (
        ...
        <div className="invite-toolbar horizontal-list gap-4mm">
          ...
          <div className="horizontal-list gap-4mm">
            <FpButton busy={busy} className="danger" icon="DO_NOT_DISTURB_ON" onClick={() => onReject(inviteData)}>
              {t("room.actions.refuse")}
            </FpButton>
            <FpButton busy={busy} className="success" icon="PERSON_ADD" onClick={() => onAccept(inviteData)}>
