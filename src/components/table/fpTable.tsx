...
export default function FpTable<T>({
        ...
        cell: (props) =>
          props.row.getCanExpand() && (
            <div className="table-expand" onClick={props.row.getToggleExpandedHandler()} style={{ userSelect: "none" }}>
              <Icon className="medium" icon={props.row.getIsExpanded() ? "KEYBOARD_ARROW_UP" : "KEYBOARD_ARROW_DOWN"} />
            </div>
          ),
      }),
    []
  ...
  return (
    <div className="table-container title rounded-m" ref={tableContainerRef}>
      {enableToolbar && (
        <div className="table-toolbar horizontal-list gap-2mm">
      ...
      <div
        role="grid"
        className="table rounded-s gap-2mm"
        ref={tableRef}
        ...
        <div className="table-data rounded-s" style={{ width: tableWrapper.getTotalSize() }}>
                ...
                >
                  <div className="header-data" onClick={header.column.getToggleSortingHandler()}>
                    ...
                    {header.column.getIsPinned() && (
                      <a onClick={() => header.column.pin(false)}>
                        <Icon className="small" icon="KEEP" />
                      </a>
                    )}
                  </div>
                  <div className="spacer"></div>
                  {(header.column.columnDef.enableResizing ?? true) && (
                    <div
                      className={`table-header-resizer ${header.column.getIsResizing() ? "resizing" : ""}`}
          ...
          {tableWrapper.getRowModel().rows.map((row) => (
            <Fragment key={row.id}>
              ...
              {row.getIsExpanded() && getDetails && <div className="row-expanded">{getDetails(row)}</div>}
            </Fragment>
          ))}
        </div>
      </div>
      {enablePagination && (
        <div className="table-pages horizontal-list gap-4mm">
          <div className="spacer"></div>
          <FpButton
            className="page-change page-arrow"
            disabled={!tableWrapper.getCanPreviousPage()}
            icon="ARROW_BACK"
            onClick={tableWrapper.previousPage}
          ></FpButton>
          {getCountArray(pagination.pageIndex, 5, 0, tableWrapper.getPageCount()).map((i) => (
            <FpButton
              key={i}
          ...
          <FpButton
            className="page-change page-arrow"
            disabled={!tableWrapper.getCanNextPage()}
            icon="ARROW_FORWARD"
            onClick={tableWrapper.nextPage}
          ></FpButton>
          <div className="spacer"></div>
        </div>
      )}
