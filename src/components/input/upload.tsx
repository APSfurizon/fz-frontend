...
export default function Upload({
  ...
  const onFileChosen = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.currentTarget.files && e.currentTarget.files.length == 1) {
      validateImage(inputRef.current!.files![0])
        .then((image) => {
          if (requireCrop) {
  ...
  const onFileUpload = (image: ImageBitmap) => {
    ...
    cropPromise
      .then((imageBlob) => {
        ...
        scaleBlob(
          imageBlob,
          isProfile ? PROFILE_UPLOAD_MAX_SIZE : FULL_UPLOAD_MAX_WIDTH,
  ...
  return (
    <>
      ...
      <div>
        ...
        <div className="upload-container vertical-list align-items-center rounded-l gap-2mm">
          ...
          <div className="vertical-list gap-2mm">
            ...
            {!media && (
              <FpButton
                title={t("components.upload.open")}
                onClick={() => openFileDialog()}
                icon="CLOUD_UPLOAD"
                disabled={readonly || formDisabled}
                busy={isBusy}
              >
                {!media && t("components.upload.open")}
              </FpButton>
            )}
            {/* Delete button */}
            {(media || previewUrl) && (
              <FpButton
                title={t("components.upload.delete")}
                className="danger"
      ...
      <Modal
        ...
        open={cropDialogOpen}
        title={cropTitle ?? t("components.upload.crop")}
        onClose={() => onCropCanceled()}
        zIndex={505}
      >
        {imageToCrop && (
          <Cropper
        ...
        <div className="bottom-toolbar">
          ...
          <div className="spacer"></div>
          <FpButton
            title={t("components.upload.upload")}
            onClick={() => onFileUpload(imageToCrop!)}
            icon="CLOUD_UPLOAD"
            disabled={readonly || formDisabled}
            busy={isBusy}
          >
            {!media && t("components.upload.upload")}
          </FpButton>
        </div>
      </Modal>
    </>
