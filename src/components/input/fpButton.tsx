...
export default function FpButton(props: Readonly<FpButtonProps>) {
    ...
    return (
        <button type={props.type ?? "button"} onClick={onClickEvent}
            ...
            {finalBusy && <Icon className={[props.iconClass ?? "medium", "loading-animation"].join(" ")} icon="PROGRESS_ACTIVITY" />}
            {!finalBusy && isCooldown && <Icon className={[props.iconClass ?? "medium"].join(" ")} icon="SNOOZE" />}
            {!finalBusy && !isCooldown && iconPresent && <Icon className={[props.iconClass ?? "medium"].join(" ")} icon={props.icon} />}
