  ...
  return (
    <>
      <div className={`fp-input ${className ?? ""}`} style={style}>
        {label && (
          <label className="title semibold small margin-bottom-1mm" style={labelStyle} htmlFor={inputFieldName}>
            {label}
          </label>
        )}
        <div
        ...
        >
          {prefix && <span className="title small color-subtitle">{prefix}</span>}
          {icon && <Icon style={{ marginLeft: "0.25em", marginRight: "0" }} className="average" icon={icon} />}
          <input
            readOnly={readOnly}
            required={isRequired}
            name={fieldName}
          ...
          <span className={`${isBusy || isPassword ? "icon-container" : ""}`}>
            {isBusy && <Icon className="medium loading-animation" icon="PROGRESS_ACTIVITY" />}
            {!isBusy && isPassword && (
              <a
                style={{ cursor: "pointer", visibility: showViewPassword ? "visible" : "hidden" }}
                onClick={() => setVisiblePassword(!visiblePassword)}
              >
                <Icon className="medium" icon={visiblePassword ? "VISIBILITY" : "VISIBILITY_OFF"} />
              </a>
            )}
          </span>
        </div>
