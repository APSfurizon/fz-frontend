...
const CONV_DAYS = [new Date(2026, 5, 2), new Date(2026, 5, 3), new Date(2026, 5, 4), new Date(2026, 5, 5)];
  ...
  if (isNarrowScreen) {
    return (
      <div className="schedule-wrapper mobile">
        <MobileDayTabs date={currentDate} onDateChange={handleNavigate} dateFnsLocale={dateFnsLocale} />
          ...
          {currentDayEvents.map((event) => {
            ...
            return (
              ...
              >
                ...
                <div className={`schedule-mobile-dot ${isNow ? "now" : ""} ${isPast ? "past" : ""}`} />
                ...
                >
                  <div className="schedule-mobile-card-title">
                    {event.titleEmote && <span aria-hidden="true">{event.titleEmote} </span>}
                    {event.title}
                  </div>
                  {room?.resourceTitle && (
  ...
  return (
    <div className="schedule-wrapper">
      <Calendar
        ...
        startAccessor="start"
        endAccessor="end"
        tooltipAccessor={(event) => (event.cancellato ? `${event.title} - CANCELLATO` : event.title)}
